...
class SoftDecisionTree(nn.Module):
    ...
    def collect_parameters(self):
        ...
        print(f"total no of leaf nodes are {leaf_counter}  and non-leaf nodes are {node_counter} for depth {self.depth}")
    ...
    def soft_forward_val_prefs(self,val_pairwise_demo_batch_train):
        ones = torch.ones((len(val_pairwise_demo_batch_train), 1)).to(self.device)
        self.forward_set_prob(self.root, val_pairwise_demo_batch_train, ones)
        val_loss_tree = self.get_loss()
        return val_loss_tree


    def argmax_forward_val_prefs(self, val_pairwise_demo_batch_train):
        class_reward_vec = torch.tensor(self.class_reward).double().detach().cpu()
        ones = torch.ones((len(val_pairwise_demo_batch_train), 1)).to(self.device)
        self.forward_set_prob(self.root, val_pairwise_demo_batch_train,ones)
        chosen_predictors = [max(self.leaves, key=lambda leaf: leaf.path_prob[i]) for i in range(len(val_pairwise_demo_batch_train))]
        # print(chosen_predictors)
        max_Q = [predictor.forward().detach().cpu() for predictor in chosen_predictors]
        max_Q_tensor = torch.cat(max_Q, dim=0)
        maxLeaf_QR = torch.sum((torch.mul(max_Q_tensor, class_reward_vec.reshape(-1, 2))), dim=1)
