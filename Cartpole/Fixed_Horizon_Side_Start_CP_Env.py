...
class FH_SideStart_CartPoleEnv(gym.Env[np.ndarray, Union[int, np.ndarray]]):
    """
    ## Description
    ...
    **Note:** While the ranges above denote the possible values for observation space of each element,
    ...
    def __init__(self, render_mode: Optional[str] = None,side=0.0):
        self.gravity = 9.8
        self.masscart = 1.0
        self.masspole = 0.1
    ...
    def step(self, action):
        ...
        '''setting terminated permnanetly equal to False'''
    ...
    def reset(
        ...
        *,
        ...
        self.state[0] = self.state[0] + self.side
        self.steps_beyond_terminated = None
...
class FH_SideStart_CartPoleVectorEnv(VectorEnv):
    metadata = {
        "render_modes": ["human", "rgb_array"],
        "render_fps": 50,
    }
    ...
    def __init__(
        ...
        render_mode: Optional[str] = None,side=0.0):
    ...
    def step(
        ...
        '''prev done is only controlled by timesteps=200'''
        self.prev_done =  truncated

        if self.render_mode == "human":
            self.render()

        assert np.all(terminated==False)
