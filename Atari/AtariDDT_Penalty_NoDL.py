...
from utils import create_training_data ,pairwise_demons_dataloader,EarlyStopping
import argparse
import random
import gc
...
class Node():
    def __init__(self, depth, nb_classes,module_list,lmbda):
    ...
    def build_children(self, depth,module_list):
        return [Node(depth - 1, self.nb_classes,module_list,self.root_lmbda),
                Node(depth - 1, self.nb_classes,module_list,self.root_lmbda)]
    def forward(self, x):
        x=x.permute(0,3,1,2)
...
class SoftDecisionTree(nn.Module):
    def __init__(self, depth,class_reward_vector):
        ...
        self.root = Node(self.depth - 1, self.nb_classes,self.module_list,lmbda=0.05)
        self.nodes = []
        self.leaves = []
...
def train(ddt,tr_pref_trajs,tr_pref_labels, no_epochs, optimizer,val_pref_trajs,val_pref_labels,batch_size):
    ...
    for epoch in range(no_epochs):
        ...
        for i in range(0, len(tr_pref_labels), batch_size):
            ...
            torch_pref_trajs = torch.from_numpy(np.array(pref_trajs)).reshape(batch_size, 2, 25, 84, 84, 4).float().to(
                device)
            ...
            # print("penalty added to the Bradley Terry Loss is ", torch.mean(C))
        ...
        with torch.no_grad():
            ...
            for v in range(0,len(val_pref_labels),batch_size):
                ...
                val_torch_pref_trajs = torch.from_numpy(np.array(val_pref_trajs)).reshape(batch_size, 2, 25, 84, 84,
                                                                                          4).float().to(device)
                ...
                # print("value of val penalty",torch.mean(val_C))
...
if __name__=="__main__":
    ...
    training_obs, training_labels = create_training_data(demons, num_snippets, min_snippet_length, max_snippet_length,
                                                         seed=0)
